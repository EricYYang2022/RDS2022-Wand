...
private:
    ...
    Vector<float, 4> theta_conv(Vector<float, 4> motor_pos, Vector3f GR, float Theta_to_pos);
...
public:
    Vector<float, 3> whiteboard(Vector<float, 4> motor_pos, Vector<float, 3> GR, float k, float c, float a);
