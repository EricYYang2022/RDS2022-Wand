...
Vector<float, 6> wall_vec() {
  ...
  Matrix4f Tpb {{0., 0., 1., 0.17}, {1., 0., 0., -.26}, {0., 1., 0., 0.05}, {0., 0., 0., 1.}};
...
const int whiteboard_button = 15;
const int inertial_button = 14;

const int inertial_button = 16;
const int soft_reset = 17;
...
void setup() {
    ...
    pinMode(whiteboard_button, INPUT_PULLUP);
    pinMode(inertial_button, INPUT_PULLUP);
    
    pinMode(inertial_button, INPUT_PULLUP);
    pinMode(soft_reset, INPUT_PULLUP);
    attachInterrupt(digitalPinToInterrupt(soft_reset), soft_reset_int, LOW);
    l
    ...
    while (digitalRead(inertial_button)) {
...
void loop() {
  ...
  if (Serial5.available() > 0) {
    ...
    p_2 = 0;// odrive1.GetPosition(m_2)- initial_p_2;
